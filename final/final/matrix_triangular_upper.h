//
//  Filename:     matrix_triangular_upper.h
//  Programmer:   Nicholas Trampe, James Kellerman
//  Class:        CS 5201 - Clayton Price
//  Assignment:   Final - Solving Poisson's Equation
//
//  Description:  This class represents an upper triangular
//                matrix.
//
...
template <class T>
class matrix_triangular_upper : public matrix_triangular_base<T>
{
private:
...
public:
	...
	matrix_triangular_upper() : matrix_triangular_base<T>() {}
	...
	matrix_triangular_upper(const size_t aSize) : matrix_triangular_base<T>(aSize) {}

	//Description: Copy constructor
	//Pre:         none
	//Post:        copies aCopy's dimensions and data over to matrix
	matrix_triangular_upper(const matrix_triangular_upper<T>& aCopy) : matrix_triangular_base<T>(aCopy) {}
	...
	matrix_triangular_upper<T>& operator=(const matrix_base<T>& aRHS);

	//Description: Negation operator
	//Pre:         class used in template needs to overload - operator
	//Post:        returns a matrix that is the negative of aRHS
	template <class U>
	friend matrix_triangular_upper<U> operator-(const matrix_triangular_upper<U>& aRHS);

	//Description: Addition operator
	//Pre:         class used in template needs to overload + operator
	//Post:        returns a matrix that is the addition of aLHS and aRHS
	template <class U>
	friend matrix_triangular_upper<U> operator+(const matrix_triangular_upper<U>& aLHS, const matrix_triangular_upper<U>& aRHS);
	...
	template <class U>
	friend matrix_triangular_upper<U> operator*(const double& aLHS, const matrix_triangular_upper<U>& aRHS);

	//Description: Scalar Multiplication operator
	//Pre:         class used in template needs to overload * (double) operator
	//Post:        returns a matrix where all elements of aLHS are multiplied by aRHS
	template <class U>
	friend matrix_triangular_upper<U> operator*(const matrix_triangular_upper<U>& aLHS, const double& aRHS);
	...
	template <class U>
	friend vector<U> operator*(const vector<U>& aLHS, const matrix_triangular_upper<U>& aRHS);

	//Description: Vector Multiplication operator
	//Pre:         class used in template needs to overload * (vector) operator
	//Post:        returns a matrix that is the multiplication of aRHS and aLHS
	template <class U>
	friend vector<U> operator*(const matrix_triangular_upper<U>& aLHS, const vector<U>& aRHS);
};
